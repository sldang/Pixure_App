  ...
  useEffect(() => {
    ...
    const fetchPosts = async () => {
      try {
        ...
        setPosts(response.data); // Assuming response is already formatted
      } catch (error) {
    ...
    <div className="fixed top-0 left-0 w-full h-full bg-black bg-opacity-70 flex items-center justify-center z-50">
      <div className="bg-white w-4/5 h-5/6 rounded-md shadow-lg overflow-y-auto relative">
        <div className="bg-gray-100 p-4 border-b border-gray-200 text-center">
          <h1 className="text-3xl font-bold text-gray-800">{community?.name || "Community Name"}</h1>
          <p className="text-gray-600">{community?.members || 0} members</p>
          <p className="text-gray-500">{community?.description || "No description available"}</p>
        </div>
...
//         // Format posts to pass to the Post component
//         const formattedPosts = data.map((post) => ({
//           id: post._id,
//           user: post.userId.nickname || "Unknown User",
//           content: post.desc || "No content available",
//           likes: post.likes.length,
//           dislikes: 0, // Dislikes aren't present in your API response
//           comments: post.comments,
//           timestamp: new Date(post.createdAt).toLocaleString(), // Format timestamp
//         }));
